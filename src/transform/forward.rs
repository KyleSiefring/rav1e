...
($m:meta, $($s:ident),*) => {
  ...
  #[$m]
  $($s)* fn daala_fdct4<T: TxOperations>(input: &[T], output: &mut [T]) {
    assert!(input.len() >= 4);
    assert!(output.len() >= 4);
  ...
  #[$m]
  $($s)* fn daala_fdst_vii_4<T: TxOperations>(input: &[T], output: &mut [T]) {
    assert!(input.len() >= 4);
    assert!(output.len() >= 4);
  ...
  #[$m]
  $($s)* fn daala_fdct8<T: TxOperations>(input: &[T], output: &mut [T]) {
    assert!(input.len() >= 8);
    assert!(output.len() >= 8);
  ...
  #[$m]
  $($s)* fn daala_fdst8<T: TxOperations>(input: &[T], output: &mut [T]) {
    assert!(input.len() >= 8);
    assert!(output.len() >= 8);
    let mut temp_out: [T; 8] = [T::default(); 8];
    daala_fdst_iv_8(
  ...
  #[$m]
  $($s)* fn fidentity4<T: TxOperations>(input: &[T], output: &mut [T]) {
    output[..4].copy_from_slice(&input[..4]);
  }

  #[$m]
  $($s)* fn fidentity8<T: TxOperations>(input: &[T], output: &mut [T]) {
