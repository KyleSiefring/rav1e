...
pub mod native {
  ...
  fn fwd_txfm2d<T: Coefficient>(
    ...
    for r in 0..txfm_size_row {
      ...
      // Segment rows more than 32 long
      let output = &mut output[(r >= 32) as usize * 32 * output_stride..];

      for cg in (0..txfm_size_col).step_by(32) {
        // Offset by zero or half of output
        let output = &mut output[(cg >= 32) as usize * txfm_size_row * 32..];

        for c in 0..txfm_size_col.min(32) {
          output[c * output_stride + (r & 31)] = T::cast_from(row_coeffs[c]);
        }
      }
    }
  }
