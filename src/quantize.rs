...
impl QuantizationContext {
    ...
    T: Coefficient,
  {
    ...
    qcoeffs[0] = {
      let coeff: i32 = i32::cast_from(coeffs[0]) << (self.log_tx_scale as usize);
      T::cast_from(divu_pair(coeff + (coeff.signum() * self.dc_offset), self.dc_mul_add))
    };
    ...
    for (i, &pos) in (1..).zip(scan[1..].iter().take(eob)) {
      ...
      let level0 = T::cast_from(divu_pair(i32::cast_from(coeff), self.ac_mul_add));
