...
pub fn get_subset_predictors<T: Pixel>(
  ...
  // then the right and bottom edges will be duplicates of the center predictor
  // when processing in raster order.

  // left
  if tile_bo.0.x > 0 {
    add_cand(
      &mut predictors,
      tile_mvs[tile_bo.0.y + (h >> 1)][tile_bo.0.x - 1],
    );
  }
  // top
  if tile_bo.0.y > 0 {
    add_cand(
      &mut predictors,
      tile_mvs[tile_bo.0.y - 1][tile_bo.0.x + (w >> 1)],
    );
  }
  // right
