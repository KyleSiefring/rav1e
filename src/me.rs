...
fn prep_square_block_fixup<T: Pixel>(
  ...
  loop {
    ...
    if mv_size_log2 == 2 {
      for &r in inter_cfg.allowed_ref_frames() {
        for y in (0..h_in_b).step_by(mv_size).rev() {
          for x in (0..w_in_b).step_by(mv_size).rev() {
            let bo = tile_bo.with_offset(x as isize, y as isize);
            if let Some(results) = estimate_motion_rev(fi, ts, bsize, bo, r) {
              let sad = results.sad << (MAX_MIB_SIZE_LOG2 - mv_size_log2) * 2;
              save_me_stats(ts, bsize, bo, r, MEStats { mv: results.mv, sad });
...
fn estimate_motion_rev<T: Pixel>(
...
fn full_pixel_me_rev<T: Pixel>(
  ...
  let subsets = get_reverse_predictors(
    tile_bo,
    tile_me_stats,
    bsize,
    mvx_min,
  ...
  let thresh = (subsets.sad as f32 * 1.2) as u32
  ...
  if let Some(median) = subsets.median {
    let results = try_cand(&[median]);
    if results.sad < thresh {
      return Some(results);
    }
  }

  let results = try_cand(&subsets.subset_b);
  if results.sad < thresh {
    return Some(results);
  }
...
struct ReversePredictors {
  sad: u32,
  median: Option<MotionVector>,
  subset_b: ArrayVec<[MotionVector; 4]>,
}

fn get_reverse_predictors(
  tile_bo: TileBlockOffset, tile_me_stats: &TileMEStats<'_>, bsize: BlockSize,
  ...
  if tile_bo.0.y < tile_me_stats.rows() - h {
    ...
    if tile_bo.0.x < tile_me_stats.cols() - w {
      subset_b
        .push(process_cand(tile_me_stats[tile_bo.0.y + h][tile_bo.0.x + w]));
    }
  }
  ...
  ReversePredictors {
    sad: tile_me_stats[tile_bo.0.y][tile_bo.0.x].sad,
    median,
    subset_b,
  }
}
