...
unsafe fn dequantize_avx2(
  ...
  for _i in (0..area).step_by(step) {
    ...
    // the abs value, it should be possible to perform a 16-bit multiply and
    // get the highest bit by comparing coeffs to (1<<16) / quant. The would
    // cost 1 compare, 1 blend, and 1 add, but would get rid of 1 pack,
    // 2 unpacks, 1 shift, and 1 multiply.
