...
pub trait InvTxfm2D: native::InvTxfm2D {
  fn match_tx_type_neon(tx_type: TxType) -> InvTxfmFunc;
    ...
    T: Pixel,
  {
    ...
    if std::mem::size_of::<T>() == 1
      && tx_type != TxType::IDTX
      && cpu >= CpuFeatureLevel::NEON
    {
      return unsafe {
        Self::inv_txfm2d_add_neon(input, output, tx_type, bd);
      };
    }
  ...
  #[inline]
  unsafe fn inv_txfm2d_add_neon<T>(
    input: &[i32], output: &mut PlaneRegionMut<'_, T>, tx_type: TxType,
    bd: usize,
  ) where
    T: Pixel,
...
macro_rules! impl_itx_fns {
  ($TYPES:tt, $W:expr, $H:expr, [$($OPT:ident),+]) => {
    ...
    paste::item! {
      ...
      impl InvTxfm2D for crate::transform::[<Block $W x $H>] {
        $(
...
mod test {
      ...
      $(
        paste::item! {
          ...
          fn [<inv_txfm2d_add_$TYPE2 _$TYPE1 _$W x $H _$OPT>]() {
            ...
            unsafe { crate::transform::[<Block $W x $H>]::[<inv_txfm2d_add_ $OPT>](
              freq, &mut dst.as_region_mut(), $ENUM, 8
            ); }
            <crate::transform::[<Block $W x $H>] as native::InvTxfm2D>::inv_txfm2d_add(
  ...
  test_itx_fns!(
    ...
    // TODO: Enable these test cases when SIGSEGV is resolved.
