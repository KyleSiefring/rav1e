...
impl PredictionMode {
  ...
  pub fn predict_inter<'a>(
    ...
    fn get_params<'a>(rec_plane: &'a Plane, po: &PlaneOffset, mv: MotionVector) -> (i32, i32, PlaneSlice<'a>) {
      ...
      for i in 0..(1 + is_compound as usize) {
        match fi.rec_buffer.frames[fi.ref_frames[ref_frames[i] - LAST_FRAME] as usize] {
          Some(ref rec) => {
            ...
            prep_8tap(&mut tmp[i].array, src, width, height, col_frac, row_frac, bit_depth)
          }
          None => ()
        }
      }
      mc_avg(dst, &tmp[0].array, &tmp[1].array, width, height, bit_depth);
    }
  }
